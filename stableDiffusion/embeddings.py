...
def get_timestep_embedding(
    ...
    """
    print("\n=== 位置编码生成 ===")
    print(f"输入 timesteps 形状: {timesteps.shape}, dtype: {timesteps.dtype}")
    print(f"参数: embedding_dim={embedding_dim}, max_period={max_period}, downscale_freq_shift={downscale_freq_shift}")
    print(f"      scale={scale}, flip_sin_to_cos={flip_sin_to_cos}")

    # 维度验证
    assert len(timesteps.shape) == 1, "Timesteps should be a 1d-array"
    print("[1] 维度验证通过: timesteps 是 1D 张量")

    half_dim = embedding_dim // 2
    print(f"[2] 半维度计算: half_dim={half_dim} (embedding_dim={embedding_dim}//2)")

    # 频率指数计算
    exponent = -math.log(max_period) * torch.arange(
        start=0, end=half_dim, dtype=torch.float32, device=timesteps.device
    )
    print(f"[3] 初始指数形状: {exponent.shape}, 均值: {exponent.mean().item():.4f}")
    print(f"    |-- 前3个值: {exponent[:3].cpu().numpy().round(4)}")

    exponent = exponent / (half_dim - downscale_freq_shift)
    print(f"[4] 缩放后指数: 除数={half_dim - downscale_freq_shift}")
    print(f"    |-- 均值: {exponent.mean().item():.4f}, 范围: [{exponent.min().item():.4f}, {exponent.max().item():.4f}]")

    # 频率矩阵生成
    emb = torch.exp(exponent)
    print(f"[5] 指数映射后 emb 形状: {emb.shape}, 均值: {emb.mean().item():.4f}")

    # 时间步外积
    emb = timesteps[:, None].float() * emb[None, :]
    print(f"[6] 时间步外积后 emb 形状: {emb.shape}")
    print(f"    |-- 示例值 (第一个时间步): min={emb[0].min().item():.4f}, max={emb[0].max().item():.4f}")

    # 缩放
    emb = scale * emb
    print(f"[7] 应用缩放 scale={scale}")
    print(f"    |-- 缩放后均值: {emb.mean().item():.4f}")

    # 正弦/余弦拼接
    emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=-1)
    print(f"[8] 拼接后形状: {emb.shape}, 通道拆分: sin({half_dim}) + cos({half_dim})")

    # 顺序翻转
    if flip_sin_to_cos:
        # new_emb.shape  # torch.Size([2, 1, 4])
        # new_emb = [
        #     [[0.9, 0.8, 0.1, 0.2]],  # 调整后：cos在前，sin在后
        #     [[0.7, 0.6, 0.3, 0.4]]
        # ]

        print(f"[9] 执行顺序翻转 (flip_sin_to_cos=True)")
        emb = torch.cat([emb[:, half_dim:], emb[:, :half_dim]], dim=-1)
        print(f"    |-- 翻转后前3个通道均值: {emb[:, :3].mean().item():.4f}")

    # 奇数维度填充
    if embedding_dim % 2 == 1:
        print(f"[10] 奇数维度填充: 原始形状={emb.shape} → 目标维度={embedding_dim}")
        emb = torch.nn.functional.pad(emb, (0, 1, 0, 0))
        print(f"    |-- 填充后形状: {emb.shape}, 最后一列均值: {emb[:, -1].mean().item():.4f}")

    print("最终位置编码形状:", emb.shape)
...
class GaussianFourierProjection(nn.Module):
    ...
    def forward(self, x):
        print("\n=== 高斯傅里叶投影计算 ===")
        print(f"输入 x 形状: {x.shape}, 均值: {x.mean().item():.4f}, 范围: [{x.min().item():.4f}, {x.max().item():.4f}]")

        # 对数变换分支
        if self.log:
            print(f"[1] 应用对数变换 (self.log=True)")
            x_prev = x.clone()
            x = torch.log(x)
            print(f"    |-- 变换后均值: {x.mean().item():.4f}, 变化量: {x.mean() - x_prev.mean():+.4f}")
        else:
            print("[1] 跳过对数变换 (self.log=False)")

        # 投影计算
        print(f"[2] 计算 x_proj")
        print(f"    |-- self.weight 形状: {self.weight.shape}, 均值: {self.weight.mean().item():.4f}")
        x_proj = x[:, None] * self.weight[None, :] * 2 * np.pi
        print(f"    |-- x_proj 形状: {x_proj.shape}, 均值: {x_proj.mean().item():.4f}")

        # 正弦/余弦拼接
        print(f"[3] 拼接三角函数 (flip_sin_to_cos={self.flip_sin_to_cos})")
        if self.flip_sin_to_cos:
            out = torch.cat([torch.cos(x_proj), torch.sin(x_proj)], dim=-1)
            print("    |-- 顺序: cos(x_proj) → sin(x_proj)")
        else:
            out = torch.cat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)
            print("    |-- 顺序: sin(x_proj) → cos(x_proj)")

        print(f"[输出] out 形状: {out.shape}, 均值: {out.mean().item():.4f}")
